...
create_pr_for_scenario() {
    ...
    local pr_body
    pr_body="## Backport Test Scenario ${scenario_padded}
...
This scenario tests the agent ability to:
...
*Generated by \`create-test-prs.sh\` for automated backport agent evaluation*"
    ...
    local pr_number
    # gh pr create automatically detects the repo from the current directory
    pr_number=$(cd "$REPO_ROOT" && gh pr create \
        --title "$pr_title" \
        --body "$pr_body" \
        --base "$TARGET_BRANCH" \
        --head "$branch_name" \
